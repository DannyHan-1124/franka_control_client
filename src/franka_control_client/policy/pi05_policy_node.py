...
class Pi05PolicyNode:
    ...
    def _predict_action_msg(self, obs_msg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with torch.inference_mode():
            abpolicy_enabled = bool(getattr(self.policy.config, "abpolicy_enabled", False))
            if abpolicy_enabled:
                action_chunk = self.policy.predict_control_points(observation)
        ...
        action_representation = getattr(
            self.policy.config, "abpolicy_action_representation", None
        )
        reference_quaternion = None
        if abpolicy_enabled and action_representation == "cartesian_rotvec":
            state = np.asarray(obs_msg["observation.state"], dtype=np.float64).reshape(-1)
            if state.size < 7:
                raise ValueError("Cartesian ABPolicy observation state must contain an xyzw quaternion")
            reference_quaternion = state[3:7].tolist()
        return {
            "timestamp": time.time(),
            "action": action_array.tolist(),
            "shape": list(action_array.shape),
            "action_representation": "bspline_control_points" if abpolicy_enabled else "raw_actions",
            "abpolicy": (
                {
                    "past_action_steps": int(self.policy.config.abpolicy_past_action_steps),
                    "future_action_steps": int(self.policy.config.abpolicy_future_action_steps),
                    "spline_degree": int(self.policy.config.abpolicy_spline_degree),
                    "num_control_points": int(self.policy.config.abpolicy_num_control_points),
                    "num_free_control_points": int(self.policy.config.abpolicy_num_free_control_points),
